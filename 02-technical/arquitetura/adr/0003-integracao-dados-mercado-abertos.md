...
O módulo de tesouraria e investimentos (`aureus-treasury`) gerencia a custódia de ativos financeiros por meio da entidade [Investimento.java](file:///c:/Users/wende/Projects/aureus-platform/backend/aureus-shared/src/main/java/com/aureus/platform/shared/entity/Investimento.java). O modelo atual oferece suporte a diversos tipos de ativos comuns no mercado brasileiro (CDB, LCA, LCI, Tesouro Direto, Fundos de Investimento).
...
Fica decidida a inclusão de uma camada unificada de **Ingestão de Dados de Mercado Abertos** na arquitetura de dados da AUREUS, operando sob as seguintes diretrizes:
...
2. **Armazenamento Híbrido (TimescaleDB e ClickHouse)**:
   * **TimescaleDB (`aureus_timeseries`)**: Armazenar os preços unitários (PUs) diários dos títulos públicos/privados e o histórico de indexadores diários (taxa CDI e IPCA acumulado).
...
4. **Exposição no Portal de Cliente**:
   ...
   * Alimentar o modelo de recomendação de investimentos no módulo `aureus-ml` usando a base consolidada de ativos.
...
* **Base para Analytics e IA**: A base histórica de cotações servirá para treinar modelos preditivos de alocação de carteira (`aureus-ml`) e precificação de crédito (`aureus-credit`).
...
|---|---|---|---|
| `aureus-data-pipelines` | Não possui fluxos de dados de mercado externos | Implementar DAG `market_data_ingestion` conectando a APIs públicas | ⏳ Planejado |
| `aureus-treasury` | Marcação de ativos com taxa de aplicação estática | Atualizar `TesourariaAvancadaService` para usar PUs históricos do TimescaleDB | ⏳ Planejado |
| `aureus-shared` | Entidade `Investimento` não possui campos de referência externa de ativos | Adicionar campos de Ticker / ID do ativo (ex: CNPJ do fundo, Código do título Tesouro) na coluna JSON `dadosInvestimento` | ⏳ Planejado |
| Frontends (`aureus-web`/`mobile`) | Exibição básica de saldo aplicado | Adicionar gráfico comparativo de rendimento acumulado vs CDI/IPCA | ⏳ Planejado |
