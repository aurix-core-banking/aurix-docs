...
|----------|--------|-----------|
| POSTGRES_CONTAINER | aureus-postgres | Nome do container PostgreSQL |
| POSTGRES_USER | aureus | Usuario do banco |
| POSTGRES_PASSWORD | aureus123 | Senha |
...
```cron
0 2 * * * cd /caminho/para/aureus-platform/infrastructure/scripts && BACKUP_DIR=/var/backups/aureus RETENTION_DAYS=30 ./backup-postgres.sh
```

O script gera:
- `aureus_YYYYMMDD_HHMMSS.sql` (banco principal)
...
# database: aureus (padrao) ou keycloak
```

**Exemplo:**
```bash
./restore-postgres.sh /var/backups/aureus/aureus_20260203_020001.sql aureus
```
...
2. Em ambiente de teste/staging: subir um PostgreSQL limpo ou usar banco `aureus_restore_test`.
...
```bash
docker exec aureus-postgres psql -U aureus -d aureus -c "SELECT COUNT(*) FROM aureus.contas; SELECT COUNT(*) FROM aureus.transacoes;"
```

## Referencias

- Roadmap e status: [../roadmap.md](../roadmap.md)
- Runbook R6: [aureus-cloud-runbook.md](aureus-cloud-runbook.md) (Backup e restore)
