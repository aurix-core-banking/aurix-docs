# AURIX Cloud – Acesso, limites e SLA

Documento voltado ao cliente que utiliza o AURIX em modo SaaS (AURIX Cloud). Item 8.5 do roadmap.

---

## O que é o AURIX Cloud

O AURIX Cloud é a oferta em que a plataforma AURIX (core banking, PIX, crédito, regulatório, etc.) é operada por nós. Sua instituição é um tenant com ambiente dedicado: banco de dados separado e configuração própria (branding, limites, produtos habilitados), sem compartilhar dados com outras instituições.
...
- **Portal e APIs**: após a ativação do seu tenant, você receberá:
  - URL base das APIs (ex.: `https://api.aurixcloud.example.com`).
...
- **Chamadas de API por mês**: volume de requisições às APIs do AURIX Cloud no ciclo.
...
- **Criptografia**: dados em trânsito (TLS) e em repouso conforme política do AURIX Cloud e do cloud provider.
...
- Pacotes de conformidade regulatória (relatórios BACEN, SPED, etc.) são mantidos e atualizados pelo AURIX Cloud dentro do escopo do plano; ofertas adicionais (consultoria, pacotes específicos) podem ser contratadas em separado.
